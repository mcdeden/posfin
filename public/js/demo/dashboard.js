...
var Dashboard = (function() {
	...
	var _componentDaterange = function() {
		...
		$('.daterange-ranges span').html(
			moment().subtract(29, 'days').format('MMMM D') + ' - ' + moment().format('MMMM D')
		);
	};
		...
		if ($(element).length > 0) {
			...
			d3.csv('../demo_data/dashboard/traffic_sources.csv', function(error, data) {
				...
				d3Container.on('mousemove', function(d, i) {
					...
					if (
						mousex >=
						$(element).outerWidth() - $('.d3-tip').outerWidth() - margin.right - tooltipOffset * 2
					) {
						tooltip
		...
		if ($(element).length > 0) {
			...
			function redraw() {
				...
				lines.each(function(d) {
					d3
						.select(d3.select(this).selectAll('circle')[0][0])
						.on('mouseover', function(d) {
				...
				lines.each(function(d) {
					d3
						.select(d3.select(this).selectAll('circle')[0][d3.select(this).selectAll('circle').size() - 1])
						.on('mouseover', function(d) {
				...
				function mousemove() {
					...
					if (
						...
						d3Container.node().getBoundingClientRect().width -
							focusText.select('text').node().getBoundingClientRect().width -
							margin.right -
							margin.left
					) {
						focusText
		...
		if ($(element).length > 0) {
			...
			d3.csv('../demo_data/dashboard/app_sales_heatmap.csv', function(error, data) {
				...
				function resizeHeatmap() {
					...
					// Groups for each app
					svg.selectAll('.hour-group').attr('transform', function(d, i) {
						return 'translate(0, ' + (gridSize + rowGap) * i + ')';
					});
					...
					// Legend item
					svg.selectAll('.heatmap-legend-item').attr('width', gridSize).attr('x', function(d, i) {
						return gridSize * i;
					});

					// Max value text label
					svg.selectAll('.max-legend-value').attr('x', buckets * gridSize + 10);
				}
			});
		}
	};
	...
	var _RoundedProgressChart = function(element, radius, border, color, end, iconClass, textTitle, textAverage) {
		...
		if ($(element).length > 0) {
			...
			var numberText = d3.select(element).append('h2').attr('class', 'pt-1 mt-2 mb-1');
			...
			// Subtitle
			d3.select(element).append('div').attr('class', 'font-size-sm text-muted mb-3').text(textAverage);
	...
	var _BulletChart = function(element, height) {
		...
		if ($(element).length > 0) {
				...
				d3.bullet = function() {
					...
					function bullet(g) {
						g.each(function(d, i) {
							...
							var range = g.selectAll('.bullet-range').data(rangez);
							...
							var measure = g.selectAll('.bullet-measure').data(measurez);
							...
							// Add loading animation
							measure.transition().duration(duration).attr('width', w1).attr('x', reverse ? x1 : 0);
							...
							var marker = g.selectAll('.bullet-marker').data(markerz);
							...
							// Add loading animation
							marker.transition().duration(duration).attr('x1', x1).attr('x2', x1);
							...
							var tick = g.selectAll('.bullet-tick').data(x1.ticks(8), function(d) {
							...
							// Append line
							tickEnter.append('line').attr('y1', height).attr('y2', height * 7 / 6 + 3);
							...
							// Update tick line
							tickUpdate.select('line').attr('y1', height + 3).attr('y2', height * 7 / 6 + 3);

							// Update tick text
							tickUpdate.select('text').attr('y', height * 7 / 6 + 4);
							...
							function resizeBulletsCore() {
								// Layout variables
								width =
									d3.select('#bullets').node().getBoundingClientRect().width -
									margin.left -
									margin.right;
								w1 = bulletWidth(x1);
			...
			d3.json('../demo_data/dashboard/bullets.json', function(error, data) {
				...
				// SVG container
				var container = d3Container.selectAll('svg').data(data).enter().append('svg');
				...
				// Bullet title text
				title.append('text').attr('class', 'bullet-title').attr('y', -10).text(function(d) {
					return d.title;
				});
